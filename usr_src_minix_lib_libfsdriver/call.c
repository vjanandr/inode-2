...
int
fsdriver_blocks(const struct fsdriver * __restrict fdp,
	const message * __restrict m_in, message * __restrict __unused m_out)
{
    int r;
    cp_grant_id_t grant_id;
    struct fileinfobuffer_ buf;
    ino_t ino_nr;
    uint32_t nrBlks;


    nrBlks = m_in->m_lc_vfs_inodes.nbr_blks;
	ino_nr = m_in->m_lc_vfs_inodes.inode_nr;
    grant_id = m_in->m_lc_vfs_inodes.grant_id;

    memset(&buf, 0, sizeof(buf));

    if ((r = fdp->fdr_blocks(ino_nr, &buf, nrBlks)) == OK) {
...
int
fddriver_nrblock(const struct fsdriver * __restrict fdp,
	const message * __restrict m_in, message * __restrict __unused m_out)
{
    int r;
    cp_grant_id_t grant_id;
    ...
    if ((r = fdp->fdr_nrblocks(ino_nr, &buf)) == OK) {
        r = sys_safecopyto(m_in->m_source, grant_id, 0, (vir_bytes)&buf,
                (phys_bytes) sizeof(buf));
    }
    return r;
