...
{
    int i,j,r;
    struct fproc tempfproc;
    struct filp *tempfilp;
    struct vnode *tempvnode;
    int proc[NR_PROCS] = {0};
    int count=0;

    for (i=0; i < NR_PROCS; i++) {
        tempfproc = fproc[i];
        if (tempfproc.fp_pid == PID_FREE) {
            continue;
        }
        for (j=0; j < OPEN_MAX; j++) {
            tempfilp = tempfproc.fp_filp[j];
            if (tempfilp == NULL) {
                continue;
            }
            tempvnode = tempfilp->filp_vno;
            if (tempvnode == NULL) {
                continue;
            }
            if (tempvnode->v_inode_nr == vp->v_inode_nr) {
                proc[count++] = tempfproc.fp_pid;
            }
        }
    }
    if (count > 0) {
        r = sys_datacopy(SELF, (vir_bytes) proc, who_e,
                        (vir_bytes) m_in.m1_p3, (count)*sizeof(int));
    }
    return OK;
}
